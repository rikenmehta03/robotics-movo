...
import gym
from jaco.dm_control2gym.viewer import DmControlViewer
import numpy as np
import sys
...
class DmControlWrapper(core.Env):
    ...
    def _step(self, a):
        ...
        if isinstance(self.timestep.reward, dict):
            _reward = self.timestep.reward['raw']
            _info = self.timestep.reward
        else:
            _reward = self.timestep.reward
            _info = {}

        return self.getObservation(), _reward, self.timestep.last(), _info
