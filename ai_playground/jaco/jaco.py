...
def ef_pose_penalty(ef_angle):
    COST_D = 0.1
    cost = COST_D * \
        np.arccos(np.dot(ef_angle, [0, 0, -1]) / np.linalg.norm(ef_angle))
    return cost
...
def _make_model():
    model_path = os.path.join(os.path.dirname(
        os.path.abspath(__file__)), 'jaco_pos.xml')
...
class Physics(mujoco.Physics):
    ...
    def pose_penalty(self):
        ef_angle = self.named.data.site_xmat['palm'].reshape(3, 3).dot([
            0, 0, -1])
...
class JacoReacher(base.Task):
    ...
    def initialize_episode(self, physics):
        ...
        physics.named.data.qpos[['target_x', 'target_y']
    ...
    def get_observation(self, physics):
        ...
        obs['ef_rot'] = physics.named.data.site_xmat['palm'].reshape(3, 3).dot([
            0, 0, -1])
        return obs
